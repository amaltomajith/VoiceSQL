...
import { tableService, ColumnDefinition, TableDefinition, DynamicEntity } from "@/lib/table-service";
...
export default function TableAnalysisPage() {
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      const numCols = tableData.schema.filter(
        (col: ColumnDefinition) => col.type === "integer" || col.type === "float",
      );
      ...
      setError("Failed to load table data: " + (error instanceof Error ? error.message : String(error)));
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateStats = (): Stats => {
    ...
    const values = rows
      .map((row: DynamicEntity) => parseFloat(row.data[selectedColumn] || "0"))
  ...
  const prepareChartData = (): ChartDataItem[] => {
    ...
    if (
      ...
      numericColumns.some((col: ColumnDefinition) => col.name === selectedColumn)
    ) {
      const values = rows
        .map((row: DynamicEntity) => parseFloat(row.data[selectedColumn] || "0"))
        .filter((val: number) => !isNaN(val));
          ...
          <div className="relative h-80">
            <div className="absolute inset-0 flex items-center justify-center">
              <svg viewBox="0 0 100 100" className="w-full h-full">
                {
                  data.reduce<{ paths: ReactElement[]; total: number }>(
                    (elements, item, i) => {
